...
import type { FaltantePijItem, FaltantesPijPorVendedor, FaltantesPijResponse } from '../../types';
...
function downloadFaltantesCsv(faltantes: FaltantePijItem[], fuente: string) {
  const headers = ['Fecha', 'Cliente', 'Vendedor', 'Serie', 'Adhesion', 'Anexo', 'Recibo', 'Estado'];
  ...
  a.href = url;
  a.download = `pij-faltantes-${fuente.replace(/\s+/g, '-').toLowerCase()}.csv`;
  a.click();
...
export function FaltantesPijModal({
  ...
  const [tab, setTab] = useState<'faltantes' | 'vendedores' | 'ambiguos'>('faltantes');
  ...
  const faltantesFiltrados = useMemo(() => {
    ...
    return faltantes.filter((f) => {
      if (vendedorFiltro && f.vendedorExcel !== vendedorFiltro) return false;
      if (!q) return true;
      return (
  ...
  }, [faltantes, busqueda, vendedorFiltro]);
    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center sm:items-center">
      ...
      <div className="relative z-10 flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-xl sm:rounded-2xl">
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-zinc-100 px-4 py-3 sm:px-5">
          <div>
            <h2 className="text-[17px] font-semibold text-zinc-900">PIJ no cargados en el CRM</h2>
            <p className="mt-0.5 text-[12px] text-zinc-500">
              Cruza adhesiones del Excel de Caja con cierres PIJ del sistema.
              {data?.fuente ? (
                <>
                  {' '}
        ...
        <div className="shrink-0 space-y-3 border-b border-zinc-100 px-4 py-3 sm:px-5">
            ...
            <label className="cursor-pointer rounded-lg border border-dashed border-brand-300 bg-brand-50/50 px-3 py-1.5 text-[12px] font-semibold text-brand-800 hover:bg-brand-50">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  reader.onload = () => {
                    onSubirCsv(String(reader.result ?? ''), file.name);
                  };
          ...
          {data?.resumen && (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
              <ResumenCard label="Adhesiones Excel" value={data.resumen.adhesionesExcel} />
              <ResumenCard label="En CRM" value={data.resumen.matched} accent="text-emerald-700" />
              <ResumenCard label="Ambiguos" value={data.resumen.ambiguos} accent="text-amber-700" />
              <ResumenCard label="Faltantes" value={data.resumen.faltantes} accent="text-red-700" />
              <ResumenCard
                label="Vendedores"
                value={data.resumen.vendedoresConFaltantes}
          ...
          <div className="flex flex-wrap gap-2">
            {(
              [
                ['faltantes', `Faltantes (${faltantes.length})`],
                ['vendedores', `Por vendedor (${porVendedor.length})`],
            ...
            {faltantes.length > 0 && (
              <button
                type="button"
                onClick={() => downloadFaltantesCsv(faltantes, data?.fuente || mes)}
                className="ml-auto rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-[12px] font-semibold text-zinc-700 hover:bg-zinc-50"
              >
                Exportar CSV faltantes
              </button>
            )}
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3 sm:px-5">
          {isLoading && (
            <p className="py-10 text-center text-[13px] text-zinc-500">Cruzando Excel con el CRM…</p>
          )}

          {!isLoading && tab === 'faltantes' && (
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                <input
                  ...
                  className="h-9 min-w-[200px] flex-1 rounded-lg border border-zinc-200 px-3 text-[13px] focus:border-brand-400 focus:outline-none"
                />
                <select
                  value={vendedorFiltro}
                  onChange={(e) => setVendedorFiltro(e.target.value)}
                  className="h-9 rounded-lg border border-zinc-200 bg-white px-2 text-[13px] text-zinc-700"
                >
                  <option value="">Todos los vendedores</option>
                  {porVendedor.map((v: FaltantesPijPorVendedor) => (
              ...
              {faltantesFiltrados.length === 0 ? (
                <p className="py-8 text-center text-[13px] text-zinc-500">
                  {faltantes.length === 0
                    ? 'No hay adhesiones faltantes: todas están en el CRM.'
          ...
          {!isLoading && tab === 'vendedores' && (
            <div className="space-y-2">
              {porVendedor.length === 0 ? (
                <p className="py-8 text-center text-[13px] text-zinc-500">Sin vendedores con faltantes.</p>
              ) : (
                porVendedor.map((v) => (
                  <details
                    key={v.vendedor}
