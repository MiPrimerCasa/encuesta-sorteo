...
function enRangoFecha(fecha: Date, desde: Date, hasta: Date) {
  const t = fecha.getTime();
  return t >= desde.getTime() && t <= hasta.getTime();
}

function leadIngresadoEnPeriodo(lead: Lead, desde: Date, hasta: Date) {
  const alta = parseFecha(lead.fechaAlta ?? lead.fechaObtencion);
  return alta != null && enRangoFecha(alta, desde, hasta);
}

function fechaCierreLead(lead: Lead) {
  return parseFecha(
    lead.seguimiento?.fechaCierre ??
      lead.seguimiento?.creadoEn ??
      (lead.seguimiento as Record<string, unknown> | undefined)?.creado_en as string | undefined,
  );
}

function cierreEnPeriodo(lead: Lead, desde: Date, hasta: Date) {
...
/** Cierres con referidos por fecha de cierre en el rango (todos los leads). */
function contarReferidosCierresPeriodo(leads: Lead[], desde: Date, hasta: Date) {
...
function seguimientoPijEnPeriodo(lead: Lead, hist: HistorialRow[], desde: Date, hasta: Date) {
  if (lead.seguimiento?.seguimientoPijPromotor === true) {
    const fc = parseFecha(lead.seguimiento?.creadoEn);
    if (fc && enRangoFecha(fc, desde, hasta)) return true;
  }
  for (const row of hist) {
    const snap = (row as Record<string, unknown>).seguimientoSnapshot ?? row;
    const s = snap as Record<string, unknown>;
    if (!bitTrue(s.seguimientoPijPromotor ?? s.seguimiento_pij_promotor)) continue;
    const f = parseFecha(fechaHistorial(row));
...
function primeraEntrevistaEnPeriodo(lead: Lead, hist: HistorialRow[], desde: Date, hasta: Date) {
  const fechas: number[] = [];
  for (const row of hist) {
    if (!filaIndicaEntrevista(row)) continue;
    const f = parseFecha(fechaHistorial(row));
